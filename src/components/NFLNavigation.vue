    ...
    <div class="d-flex navigation-container">
      <v-list
        ...
        >
          ...
          <v-list-item-title v-show="isExpanded">{{ division.name }}</v-list-item-title>
        </v-list-item>
      </v-list>
      ...
      <v-list v-if="isExpanded" class="teams-list">
        <v-list-item
          ...
          :key="team.name"
          :to="{ name: 'team', params: { division: selectedDivision, team: team.name.toLowerCase().replace(/ /g, '-') }}"
          :active="$route.params.team === team.name.toLowerCase().replace(/ /g, '-')"
...
watch(() => route.params.division, (newDivision) => {
  if (newDivision) {
